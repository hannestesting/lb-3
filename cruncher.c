...
void query(unsigned short qid, unsigned short artist, unsigned short areltd[], unsigned short bdstart, unsigned short bdend) {
	...
	// copy the relevant knows_n 
	for (person_offset = 0; person_offset < person_length/sizeof(Person); person_offset++) {
		int knows_n = 0;
		person = &person_map[person_offset];
		if (person_offset > 0 && person_offset % REPORTING_N == 0) {
			printf("%.2f%% pass(knows.bin)\n", 100 * (person_offset * 1.0/(person_length/sizeof(Person))));
		}
		if (person_score[person_offset] > 0) {
			for (knows_offset = person->knows_first; knows_offset < person->knows_first + person->knows_n; knows_offset++) {
				if (person_score[knows_map[knows_offset]] == -1) {
					knows_n++;
				}
			}
			knows_buf[person_offset] = (unsigned int*) malloc((1+knows_n)*sizeof(unsigned int));
			knows_buf[person_offset][0] = knows_n;
			knows_n = 0;
			for (knows_offset = person->knows_first; knows_offset < person->knows_first + person->knows_n; knows_offset++) {
				if (person_score[knows_map[knows_offset]] == -1) { 
					knows_buf[person_offset][++knows_n] = knows_map[knows_offset];
				}
			}
		} 
	...
	for (friend_offset = 0; friend_offset < person_length/sizeof(Person); friend_offset++) {
		...
		for (knows_offset = knows->knows_first; 
			...
			// friendship must be mutual, lookup in copied knows_buf
			for (knows_offset2 = 1; knows_offset2 <= knows_buf[person_offset][0]; knows_offset2++) {
				if (friend_offset == knows_buf[person_offset][knows_offset2]) {
	...
	// deallocate memory
	for (person_offset = 0; person_offset < person_length/sizeof(Person); person_offset++) {
		if (knows_buf[person_offset] == NULL)  continue;
		free(knows_buf[person_offset]);
		knows_buf[person_offset] = NULL;
	}
}
...
int main(int argc, char *argv[]) {
	...
	knows_buf    = (unsigned int**)   malloc(person_length*sizeof(unsigned int*)/sizeof(Person)); 
	memset(knows_buf, 0, person_length*sizeof(unsigned int*)/sizeof(Person)); 
